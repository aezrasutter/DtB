...
def simulateData(
    ...
    stim_strengths = np.array(
        [-0.512, -0.256, -0.128, -0.064, -0.032, 0, 0.032, 0.064, 0.128, 0.256, 0.512]
    )
...
def fitData(
    ...
    theta_lo = [0, 0, 0, 0, 0.1, 0.01, -np.inf, -np.inf]
    theta_hi = [np.inf, np.inf, np.max(rt), np.inf, np.inf, 0.5, np.inf, np.inf]
    theta_init = [k, B0, Bdel, B2, tnd, tnd_sd, stim_strength_bias, y0]

    theta, fopt = fminbnd.fminbnd(
        calculateNegLL,
        theta_init,
        args=(stim_strengths, rt, ci, dt),
        LB=theta_lo,
        UB=theta_hi,
    )
...
def plotDataWithFit(data, theta, dt=0.0005):
    """
    ...
    ax_choice.add_line(plt.Line2D(stim_strengths_to_propagate, predprop, color="k"))
    ...
    ax_rt.add_line(
        plt.Line2D(
            stim_strengths_to_propagate, predrt_up, color=colors[0], marker="None"
        )
    )
    ax_rt.add_line(
        plt.Line2D(
            stim_strengths_to_propagate, predrt_lo, color=colors[1], marker="None"
        )
    )
    ...
    ax_rt.set_ylim(
        np.nanmin(np.concatenate([meanrt_up, meanrt_lo])) * 0.75,
...
def plotBeliefMap(theta, dt=0.0005):
    """
    Given a diffusion to bound model with quadratically collapsing bounds
    ...
    drifts, t, Bup, Blo, y, yinit = discretize(theta, ustim_strengths, 5, dt)
    ...
    pstim_xt = densities / (np.sum(densities + eps, axis=0)[np.newaxis, :, :])
    ...
    p_correct_lo = np.sum(pstim_t[:, neg_sel], axis=1) + 0.5 * pstim_t[:, midx]

    densities = D["up"]["pdf_t"] * priors
    pstim_t = densities / np.sum(densities + eps, axis=1)[:, np.newaxis]
    p_correct_up = np.sum(pstim_t[:, pos_sel], axis=1) + 0.5 * pstim_t[:, midx]
    ...
    b_correct[:] = np.nan
    b_correct[(idx_lo - n) : idx_lo, :] = np.tile(p_correct_lo[np.newaxis, :], [n, 1])
    b_correct[idx_up : idx_up + n, :] = np.tile(p_correct_up[np.newaxis, :], [n, 1])
    ...
    im = ax.imshow(
        ...
        vmax=1,
        cmap="Purples",
        extent=[t[0], t[-1], y[-1], y[0]],
    )
    ax.imshow(
        b_correct, vmin=0.5, vmax=1, cmap="Purples", extent=[t[0], t[-1], y[-1], y[0]]
    )
    ...
    tmax = t[np.where(np.sum(p_correct_notabs, axis=0) == 0)[0][0]]
    im = ax.imshow(
        np.log(p_correct_notabs / (1 - p_correct_notabs)),
        vmin=0,
        cmap="RdPu",
        extent=[t[0], t[-1], y[-1], y[0]],
    )
    ax.imshow(
        np.log(b_correct / (1 - b_correct)),
        vmin=0,
        cmap="RdPu",
        extent=[t[0], t[-1], y[-1], y[0]],
    )
...
def calculateNegLL(theta, stim_strengths, rt, ci, dt=0.0005):
    ...
    P_UP = np.zeros(
        [D["up"]["pdf_t"].shape[0] + r.shape[0] - 1, D["up"]["pdf_t"].shape[1]]
    )
    P_LO = np.zeros(
        [D["lo"]["pdf_t"].shape[0] + r.shape[0] - 1, D["up"]["pdf_t"].shape[1]]
    )
    ...
    print(
        "NegLL = {:.3f}, k = {:.2f}, B0 = {:.2f}, Bdel = {:.3f}, B2 = {:.4f} ".format(
            neglogl, theta[0], theta[1], theta[2], theta[3]
        )
        + "tnd = {:.2f}, tnd_sd = {:.2f}, stim_bias = {:.5f}, y0 = {:.5f}".format(
...
if __name__ == "__main__":
    ...
    print("---" * 20)
    print("---" * 20)
    print("True k: {:.3f}, Fitted k: {:.3f}".format(theta[0], fitted_theta[0]))
    print("True B0: {:.3f}, Fitted B0: {:.3f}".format(theta[1], fitted_theta[1]))
    print("True Bdel: {:.3f}, Fitted Bdel: {:.3f}".format(theta[2], fitted_theta[2]))
    print("True B2: {:.3f}, Fitted B2: {:.3f}".format(theta[3], fitted_theta[3]))
    print("True tnd: {:.3f}, Fitted tnd: {:.3f}".format(theta[4], fitted_theta[4]))
    print(
        "True tnd_sd: {:.3f}, Fitted tnd_sd: {:.3f}".format(theta[5], fitted_theta[5])
    )
    print("True bias: {:.4f}, Fitted bias: {:.4f}".format(theta[6], fitted_theta[6]))
    print(
        "True offset: {:.4f}, Fitted offset: {:.4f}".format(theta[7], fitted_theta[7])
    )
    print("---" * 20)
    print("---" * 20)
